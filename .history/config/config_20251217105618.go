...
func (c *ChainConfig) ToProto() *pb.ChainConfig {
	return &pb.ChainConfig{
		...
		SyncIntervalMs: c.SyncIntervalMs, // 修改：直接使用 uint64 值，不再转换
...
func (c *ChainConfig) FromProto(pb *pb.ChainConfig) {
	...
	c.SyncIntervalMs = pb.SyncIntervalMs // 修改：直接赋值 uint64 值，不再转换
...
func (c *ChainConfig) Validate() error {
	...
	// 1. 基础策略参数校验（SyncIntervalMs 现在是 uint64 类型）
	if c.SyncIntervalMs == 0 { // 修改：检查是否为 0，而不是 <= 0
		return errors.New("Chain.SyncIntervalMs must be positive")
	}
...
func DefaultChainConfig() ChainConfig {
	return ChainConfig{
		...
		SyncIntervalMs: 500, // 修改：直接使用毫秒值 500，而不是 time.Duration
...
func (g *GlobalChainConfig) ToProto() *pb.GlobalChainConfig {
	return &pb.GlobalChainConfig{
		...
		DynamicAdjustIntervalMs: g.DynamicAdjustInterval, // 直接使用 uint64 值
...
func (g *GlobalChainConfig) FromProto(pb *pb.GlobalChainConfig) {
	...
	g.DynamicAdjustInterval = pb.DynamicAdjustIntervalMs // 直接赋值 uint64 值
