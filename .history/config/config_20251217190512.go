...
// 	// config.go 中的 GlobalChainConfig.DynamicAdjustInterval 是 time.Duration 类型，而 chainconfig.pb.go 中的 DynamicAdjustIntervalMs 是 uint64 类型（毫秒）。
// 	// 将 config.go 中的 DynamicAdjustInterval 类型改为 uint64 以保持与 protobuf 定义的一致性，因为：Protobuf 不支持 time.Duration 类型，通常使用毫秒数表示时间间隔，所以不改chainconfig
// 	DynamicAdjustInterval uint64 `json:"dynamic_adjust_interval"` // 从 time.Duration 改为 uint64（毫秒）
// 	ServiceDiscoveryAddr  string `json:"service_discovery_addr"`  // 服务发现地址（生产环境必填）

// 	SyncIntervalMs uint64 `json:"sync_interval_ms"`
...
type ChainConfig struct {
	...
	// 核心开关与拓扑（与 protobuf 静态拓扑配置对齐）
	EnableChain           bool         `json:"enable_chain"`            // 是否启用链式复制
	PrevShardId           uint64       `json:"prev_shard_id"`           // 上游分片ID
	NextShardId           uint64       `json:"next_shard_id"`           // 下游分片ID
	AvailableShards       []uint64     `json:"available_shards"`        // 候选分片列表
	ChainRole             pb.ChainRole `json:"chain_role"`              // 链角色
	LocalShardId          uint64       `json:"local_shard_id"`          // 本地分片ID
	LocalLeaderId         uint64       `json:"local_leader_id"`         // 本地领导者ID
	LocalLeaderAddr       string       `json:"local_leader_addr"`       // 本地领导者地址
	LocalLeaderTerm       uint64       `json:"local_leader_term"`       // 本地领导者任期
	FromIndex             uint64       `json:"from_index"`              // 起始索引
	ToIndex               uint64       `json:"to_index"`                // 结束索引
	StateVector           []byte       `json:"state_vector"`            // 状态向量
	SyncIndex             uint64       `json:"sync_index"`              // 同步索引
	Timestamp             uint64       `json:"timestamp"`               // 时间戳
	DynamicAdjustInterval uint64       `json:"dynamic_adjust_interval"` // 动态调整间隔
	SyncIntervalMs        uint64       `json:"sync_interval_ms"`        // 同步间隔
	MaxRetryCount         uint64       `json:"max_retry_count"`         // 最大重试次数
}
...
func (c *ChainConfig) IsEmpty() bool {
	return c.PrevShardId == 0 && // 校验上游分片ID
		c.NextShardId == 0 && // 校验下游分片ID
		len(c.AvailableShards) == 0 && // 校验候选分片列表
...
func (c *ChainConfig) ToProto() *pb.ChainConfig {
	...
	return &pb.ChainConfig{
		...
		PrevShardId:             c.PrevShardId,
		NextShardId:             c.NextShardId,
		AvailableShards:         c.AvailableShards,
		ChainRole:               c.ChainRole,
		LocalShardId:            c.LocalShardId,
		LocalLeaderId:           c.LocalLeaderId,
		LocalLeaderAddr:         c.LocalLeaderAddr,
		LocalLeaderTerm:         c.LocalLeaderTerm,
		FromIndex:               c.FromIndex,
		ToIndex:                 c.ToIndex,
		StateVector:             c.StateVector,
		SyncIndex:               c.SyncIndex,
		Timestamp:               c.Timestamp,
		DynamicAdjustIntervalMs: c.DynamicAdjustInterval,
...
func (c *ChainConfig) FromProto(pb *pb.ChainConfig) {
	...
	c.PrevShardId = pb.PrevShardId
	c.NextShardId = pb.NextShardId
	c.AvailableShards = pb.AvailableShards
	c.ChainRole = pb.ChainRole
	c.LocalShardId = pb.LocalShardId
	c.LocalLeaderId = pb.LocalLeaderId
	c.LocalLeaderAddr = pb.LocalLeaderAddr
	c.LocalLeaderTerm = pb.LocalLeaderTerm
	c.FromIndex = pb.FromIndex
	c.ToIndex = pb.ToIndex
	c.StateVector = pb.StateVector
	c.SyncIndex = pb.SyncIndex
	c.Timestamp = pb.Timestamp
	c.DynamicAdjustInterval = pb.DynamicAdjustIntervalMs
	c.SyncIntervalMs = pb.SyncIntervalMs
	c.MaxRetryCount = pb.MaxRetryCount
}

// Validate validates the ChainConfig instance.
func (c *ChainConfig) Validate() error {
	if !c.EnableChain {
		return nil // 如果未启用链式复制，则不进行验证
	}
...
// 新增：验证全局链式配置的辅助方法
// func (g *GlobalChainConfig) Validate() error {
// 	if g.MaxChainLength == 0 {
// 		return errors.New("MaxChainLength must be positive")
// 	}
// 	if g.DynamicAdjustInterval == 0 {
// 		return errors.New("DynamicAdjustInterval must be positive")
// 	}
// 	if g.SyncIntervalMs == 0 {
// 		return errors.New("SyncIntervalMs must be positive")
// 	}
// 	if g.MaxRetryCount == 0 {
// 		return errors.New("MaxRetryCount must be greater than 0")
// 	}
// 	return nil
// }
...
// 		EnableChain:             g.EnableChain,
// 		MaxChainLength:          g.MaxChainLength,
// 		DynamicAdjustIntervalMs: g.DynamicAdjustInterval,
// 		ServiceDiscoveryAddr:    g.ServiceDiscoveryAddr,
// 		SyncIntervalMs:          g.SyncIntervalMs,
// 		MaxRetryCount:           g.MaxRetryCount,
// 	}
// }

// // FromProto 从 raftpb.GlobalChainConfig 加载配置
// func (g *GlobalChainConfig) FromProto(pb *pb.GlobalChainConfig) {
// 	g.EnableChain = pb.EnableChain
// 	g.MaxChainLength = pb.MaxChainLength
// 	g.DynamicAdjustInterval = pb.DynamicAdjustIntervalMs
// 	g.ServiceDiscoveryAddr = pb.ServiceDiscoveryAddr
// 	g.SyncIntervalMs = pb.SyncIntervalMs
// 	g.MaxRetryCount = pb.MaxRetryCount
// }
...
// 		DynamicAdjustInterval: 30000, // 默认30秒调整一次（30000毫秒）
// 		ServiceDiscoveryAddr:  "",    // 默认不启用服务发现
// 		SyncIntervalMs:        500,   // 默认同步间隔500ms
...
func (c *NodeHostConfig) validateChainConfig() error {
	...
	// 验证每个链配置的 DynamicAdjustInterval
	for shardID, cfg := range c.ChainConfigs {
		if cfg.DynamicAdjustIntervalMs == 0 { // 检查是否为0
			return errors.Errorf("DynamicAdjustInterval must be positive for shard %d", shardID)
		}
		// 检查是否存在循环引用
