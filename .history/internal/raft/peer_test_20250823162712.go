...
// 新增：测试用 ILeaderResolver 实现（仅用于满足接口要求）
type testLeaderResolver struct{}

// 新增
func (t *testLeaderResolver) Resolve(shardID uint64, replicaID uint64) (string, string, error) {
	return "", "", nil // 测试环境返回空值
}
func (t *testLeaderResolver) Add(shardID uint64, replicaID uint64, target string) {}
func (t *testLeaderResolver) SetShardLeader(shardID uint64, leaderID uint64)      {}

// 修正：实现带错误返回值的 GetShardLeader 方法
func (t *testLeaderResolver) GetShardLeader(shardID uint64) (uint64, error) {
	return 0, nil // 测试环境返回默认值和空错误
}
...
func TestRaftAPINodeStep(t *testing.T) {
	for i := range pb.MessageType_name {
		s := NewTestLogDB()
		// 创建测试解析器实例
		resolver := &testLeaderResolver{}
		// 补充第7个参数：resolver
		// rawNode := Launch(newTestConfig(1, 10, 1), s, nil, []PeerAddress{{ReplicaID: 1}}, true, true)
		rawNode := Launch(newTestConfig(1, 10, 1), s, nil, []PeerAddress{{ReplicaID: 1}}, true, true, resolver)
...
func TestRaftAPIRequestLeaderTransfer(t *testing.T) {
	...
	resolver := &testLeaderResolver{}
	// 补充第7个参数：resolver
	p := Launch(newTestConfig(1, 10, 1), s, nil, []PeerAddress{{ReplicaID: 1}}, true, true, resolver)
...
func TestRaftAPIReadIndex(t *testing.T) {
	...
	resolver := &testLeaderResolver{}
	// 补充第7个参数：resolver
	rawNode := Launch(c, s, nil, []PeerAddress{{ReplicaID: 1}}, true, true, resolver)
	rawNode.raft.hasNotAppliedConfigChange = rawNode.raft.testOnlyHasConfigChangeToApply
...
func TestRaftAPIStatus(t *testing.T) {
	...
	resolver := &testLeaderResolver{}
	// 补充第7个参数：resolver
	rawNode := Launch(newTestConfig(1, 10, 1), storage, nil, []PeerAddress{{ReplicaID: 1}}, true, true, resolver)
...
func TestRaftAPIInvalidReplicaIDCausePanicInLaunch(t *testing.T) {
	...
	resolver := &testLeaderResolver{}
	// 补充第7个参数：resolver
	Launch(config.Config{}, nil, nil, nil, true, true, resolver)
}
...
func TestRaftAPIInvalidInputToLaunchCausePanic(t *testing.T) {
	...
	resolver := &testLeaderResolver{}
	// 补充第7个参数：resolver
	Launch(newTestConfig(1, 10, 1), storage, nil, []PeerAddress{}, true, true, resolver)
}
...
func TestRaftAPIRestart(t *testing.T) {
	...
	resolver := &testLeaderResolver{}
	// 补充第7个参数：resolver
	rawNode := Launch(newTestConfig(1, 10, 1), storage, nil, nil, true, false, resolver)
	rawNode.raft.hasNotAppliedConfigChange = rawNode.raft.testOnlyHasConfigChangeToApply
	ud, err := rawNode.GetUpdate(true, 0)
	if err != nil {
		t.Fatalf("unexpected error %v", err)
...
func TestRaftAPIRestartFromSnapshot(t *testing.T) {
	...
	resolver := &testLeaderResolver{}
	// 补充第7个参数：resolver
	rawNode := Launch(newTestConfig(1, 10, 1), s, nil, nil, true, false, resolver)
	rawNode.raft.hasNotAppliedConfigChange = rawNode.raft.testOnlyHasConfigChangeToApply
	ud, err := rawNode.GetUpdate(true, 0)
	if err != nil {
		t.Fatalf("unexpected error %v", err)
...
func TestRaftAPIStepOnLocalMessageWillPanic(t *testing.T) {
	...
	resolver := &testLeaderResolver{}
	// 补充第7个参数：resolver
	p := Launch(newTestConfig(1, 10, 1), storage, nil, []PeerAddress{{ReplicaID: 1}}, true, true, resolver)
