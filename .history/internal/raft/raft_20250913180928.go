...
const (
	...
	numMessageTypes uint64 = 33 // 修改：29 → 33，覆盖所有MessageType枚举值（0-32）（更新为33种，含链式连接相关类型）
)
...
func (r *raft) abortLeaderTransfer() {
	...
	if r.leaderTransferTarget != NoNode {
		...
		r.chainDownstream = struct{ ShardID, LeaderID uint64 }{} // 清空下游信息
	}
	// 3. 重置领导者转移目标（核心状态清理）
...
// 新增：从外部配置获取链式下一跳分片ID（需实现配置加载逻辑）
func (r *raft) getNextShardInChain(currentShardID uint64) uint64 {
	// 示例：从集群配置中读取当前分片的下游映射
	// 实际实现需对接配置中心（如etcd/consul）或本地配置文件
...
func (r *raft) checkChainHealth() {
	...
	r.send(msg)
	// 2. 若超时未收到 Pong，触发重连逻辑（略）
}

// 新增：定期检查链式连接健康状态（集成到领导者定时任务）
func (r *raft) leaderChainHealthTick() {
	if !r.isLeader() || r.chainUpstream.LeaderID == 0 {
		return
	}
	// 发送健康检查Ping消息
	r.send(pb.Message{
	...
	// 检查下游连接是否超时
	if r.chainDownstream.LeaderID != 0 && r.chainDownstream.lastPingAck.Add(30*time.Second).Before(time.Now()) {
