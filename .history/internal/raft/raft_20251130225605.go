...
// 	nextShardID := chainCfg.NextShardId
...
// 		// 使用备用分片列表中的第一个分片
// 		if len(chainCfg.AvailableShards) > 0 {
// 			nextShardID = chainCfg.AvailableShards[0]
// 		}
// 	}

// 	// === 3. 参数合法性校验 ===
// 	if nextShardID == 0 {
// 		plog.Warningf("%s no valid next shard (default: %d, available: %v)",
// 			r.describe(), chainCfg.NextShardId, chainCfg.AvailableShards)
// 		return nil
// 	}
...
// 		Type:     pb.LEADER_CHAIN_CONNECT,
// 		From:     r.replicaID,
// 		To:       0,                          // 发送给目标分片的领导者
// 		ShardID:  nextShardID,                // 目标分片ID
...
// 	// === 6. 发送请求与重试逻辑 ===
// 	if err := r.transport.Send(connectMsg); err != nil {
...
// 			retryDelay := chainCfg.SyncIntervalMs / 2 // 指数退避简化为固定间隔的1/2
...
// 	r.chainState.nextShardID = nextShardID
