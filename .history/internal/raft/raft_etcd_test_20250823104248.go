...
// 新增添加模拟的LeaderResolver实现
type mockLeaderResolver struct{}

// 新增
//
//	func (m *mockLeaderResolver) ResolveLeader(groupID uint64) (uint64, error) {
//		return 0, nil // 测试环境返回默认值
//	}
//
// 修改方法名 ResolveLeader -> Resolve 以匹配接口要求
func (m *mockLeaderResolver) Resolve(shardID, replicaID uint64) (string, string, error) {
	return "", "", nil // 测试环境返回空字符串和 nil 错误
}

// 新增 SetShardLeader 方法以完整实现 ILeaderResolver 接口
func (m *mockLeaderResolver) SetShardLeader(shardID, leaderID uint64) error {
	return nil // 测试环境无需实际实现，返回 nil 即可
}
...
func testCampaignWhileLeader(t *testing.T) {
	...
	// 新增 添加第三个参数：mockLeaderResolver实例
	r := newRaft(cfg, s, &mockLeaderResolver{})
