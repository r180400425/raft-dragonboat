...
// 新增在测试文件开头添加测试用解析器定义（如果尚未定义）
// type testLeaderResolver struct{}

// func (t *testLeaderResolver) Resolve(shardID uint64, leaderID uint64) (addr string, connKey string, err error) {
//     return "", "", nil
// }
...
func testCampaignWhileLeader(t *testing.T) {
	...
	// 新增：修改测试函数中的 newTestRaft 调用，添加解析器参数 (以 TestLeaderElectionWithCheckQuorum 为例)
	// 创建测试用领导者解析器实例
	resolver := &testLeaderResolver{}
	r := newRaft(cfg, s, resolver)
...
func entsWithConfig(configFunc func(config.Config), terms ...uint64) *raft {
	...
	// 创建测试用领导者解析器实例
	resolver := &testLeaderResolver{}
	// 新增 resolver 作为第三个参数传递给 newRaft
	sm := newRaft(cfg, storage, resolver)
...
func newNetworkWithConfig(configFunc func(config.Config), peers ...stateMachine) *network {
	...
	for j, p := range peers {
		...
		case nil:
			...
			resolver := &testLeaderResolver{}
			// 新增 resolver 作为第三个参数传递给 newRaft
			sm := newRaft(cfg, nstorage[id], resolver)
...
func newTestRaft(id uint64, peers []uint64, election, heartbeat int, logdb ILogDB) *raft {
	...
	r := newRaft(newTestConfig(id, election, heartbeat), logdb, resolver)
