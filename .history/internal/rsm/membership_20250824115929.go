...
// 作用：这是状态机（RSM）内部用于管理成员信息的辅助结构体，不直接定义 Membership 字段，而是通过 members 字段持有 raftpb.Membership 实例，实现成员变更的校验、深拷贝等逻辑。
type membership struct {
	members   pb.Membership // 实际存储 raftpb.Membership 实例
	shardID   uint64
	replicaID uint64
	ordered   bool // 是否启用有序成员变更
}
