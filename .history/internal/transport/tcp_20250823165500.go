...
import (
	...
	"github.com/lni/dragonboat/v4/raft"
...
type TCP struct {
	...
	encrypted      bool
	// 新增：Shard 领导者解析器，用于查询目标 Shard 的当前领导者
	// resolver raftio.ShardLeaderResolver
	resolver raft.ILeaderResolver
}

var _ raftio.ITransport = (*TCP)(nil)

// NewTCPTransport creates and returns a new TCP transport module.
// 新增参数：resolver - Shard 领导者解析器实例
func NewTCPTransport(nhConfig config.NodeHostConfig,
	requestHandler raftio.MessageHandler,
	chunkHandler raftio.ChunkHandler,
	resolver raft.ILeaderResolver) raftio.ITransport {
	return &TCP{
		nhConfig:       nhConfig,
...
// SendMessageToShardLeader 向目标 Shard 的当前领导者发送 Raft 消息
func (t *TCP) SendMessageToShardLeader(targetShardID uint64, msg pb.Message) error {
	// 1. 校验领导者解析器是否初始化
	if t.resolver == nil {
		return errors.New("shard leader resolver not initialized")
	}

	// 2. 查询目标 Shard 的当前领导者 ID
	leaderID, err := t.resolver.GetLeader(targetShardID)
	if err != nil {
		return errors.Wrapf(err, "failed to resolve leader for shard %d", targetShardID)
	}
	if leaderID == raftio.NoLeader {
		return errors.Errorf("no leader found for shard %d", targetShardID)
	}

	// 3. 查询领导者节点的网络地址（需根据工程实际节点发现机制实现）
	// 示例：从节点注册表获取 leaderID 对应的地址（此处需替换为实际逻辑）
	leaderAddr, err := t.getLeaderAddress(leaderID)
	if err != nil {
		return errors.Wrapf(err, "failed to get address for leader %d", leaderID)
	}

	// 4. 完善消息元信息并封装为消息批
	msg.To = leaderID           // 目标设为领导者 ReplicaID
	msg.ShardID = targetShardID // 明确消息所属 Shard
	batch := pb.MessageBatch{
		// Messages: []pb.Message{msg},
		Requests: []pb.Message{msg},
	}
	...
	if err != nil {
		return errors.Wrapf(err, "failed to connect to leader %s", leaderAddr)
	}
	defer conn.Close()

	if err := conn.SendMessageBatch(batch); err != nil {
		return errors.Wrapf(err, "failed to send message to leader %s", leaderAddr)
	}
	return nil
}

// 辅助方法：获取领导者节点的网络地址（需根据工程实际节点注册表实现）
func (t *TCP) getLeaderAddress(leaderID uint64) (string, error) {
	// 示例逻辑：查询节点注册表获取 leaderID 对应的地址
	// 实际实现需替换为工程的节点发现逻辑（如从配置中心或本地缓存查询）
	return "127.0.0.1:0", nil // 占位：替换为实际地址查询逻辑
}
