...
func (t *Transport) send(req pb.Message) (bool, failedSend) {
	...
	// 移除 localShardID 判断，无论是否本地分片均通过 resolver 解析
	// 核心：通过 resolver 动态解析目标分片领导者，支持链式路由
	// 新增：核心修改：跨分片消息自动路由至目标分片领导者
	// if shardID != t.localShardID { // 目标分片非本地分片 → 跨分片通信
	// 	toReplicaID = 0 // 将目标节点 ID 设为 0，触发领导者查询
	// 	plog.Debugf("cross-shard message detected: local %d → target shard %d, querying leader",
	// 		t.localShardID, shardID)
	// }
