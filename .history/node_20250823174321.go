...
func newNode(peers map[uint64]string,
	...
	// 在 newNode 函数中初始化 raftEvents 时补充缺失的参数
	// 原代码：
	// rn.raftEvents = newRaftEventListener(config.ShardID, config.ReplicaID, nhConfig.EnableMetrics, liQueue)
	// 修改后：
	rn.raftEvents = newRaftEventListener(
		config.ShardID,
		config.ReplicaID,
		nhConfig.EnableMetrics,
		liQueue,
		nhConfig.DeploymentID, // 补充：DeploymentID（uint64 类型）
		nil)                   // 补充：Transport 实例（若无需实际网络通信可传 nil）
...
func (n *node) startRaft(cfg config.Config,
	...
	n.p = raft.Launch(cfg, n.logReader, n.raftEvents, pas, initial, newNode, n.nodeRegistry.(raft.ILeaderResolver))
	/*
		错误提示 raftio.INodeRegistry does not implement raft.ILeaderResolver (missing method GetShardLeader) 表明 n.nodeRegistry 虽然实际类型是 *registry.Registry（已实现 raft.ILeaderResolver），但声明类型为 raftio.INodeRegistry，导致编译器无法识别其 GetShardLeader 方法。需通过类型断言将其转换为 raft.ILeaderResolver 接口。

		类型断言：n.nodeRegistry.(raft.ILeaderResolver) 将 nodeRegistry 从 raftio.INodeRegistry 显式转换为 raft.ILeaderResolver，编译器即可识别其实现了 GetShardLeader 方法。
		安全性：Registry 结构体已通过 var _ raft.ILeaderResolver = (*Registry)(nil) 确保实现 raft.ILeaderResolver 接口，因此类型断言必然成功，无需错误处理。
		接口兼容性：此修改不影响 nodeRegistry 原有的 raftio.INodeRegistry 功能，仅补充了 raft.ILeaderResolver 接口的方法暴露。
