...
func (nh *NodeHost) checkChainLength() {
	...
	if currentLen < int(nh.chainConfig.leaderCount) {
		plog.WARNING.Printf("领导者链条长度不足: 当前 %d, 预期 %d", currentLen, nh.chainConfig.leaderCount)
	} else if currentLen > int(nh.chainConfig.leaderCount) {
		plog.WARNING.Printf("领导者链条长度过长: 当前 %d, 预期 %d", currentLen, nh.chainConfig.leaderCount)
	} else {
