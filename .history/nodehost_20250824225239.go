...
func NewNodeHost(nhConfig config.NodeHostConfig) (*NodeHost, error) {
	...
	// 新增初始化领导者链条配置，从配置中读取
	nh.chainConfig.leaderCount = nhConfig.Expert.ChainLeaderCount
	nh.chainConfig.followerCount = nhConfig.Expert.ChainFollowerCount
	nh.leaderChain = make([]uint64, 0, nh.chainConfig.leaderCount)
