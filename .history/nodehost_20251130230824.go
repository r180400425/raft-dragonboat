...
// 【新增】分片链式状态结构体（替代原领导者级连接状态）
// type ShardChainState struct {
// 	upstreamShardID    uint64              // 上游分片ID（固定，不随领导者变化）
// 	downstreamShardID  uint64              // 下游分片ID（固定，不随领导者变化）
// 	upstreamLeaderID   uint64              // 上游分片当前领导者ID（动态更新）
// 	downstreamLeaderID uint64              // 下游分片当前领导者ID（动态更新）
// 	stateVector        pb.ShardStateVector // 分片状态向量（用于一致性检测）
// 	lastSyncTime       time.Time           // 最后状态同步时间
// 	healthy            bool                // 分片连接健康状态
// }
...
func NewNodeHost(nhConfig config.NodeHostConfig) (*NodeHost, error) {
	...
	// 【新增】初始化分片级链式状态
	// nh.shardChain.config = make(map[uint64]pb.ChainConfig)
	// nh.shardChain.state = make(map[uint64]ShardChainState)
	// nh.shardChain.resolver = NewDefaultShardLeaderResolver() // 初始化领导者解析器

	// // 加载默认分片链式配置（示例：从 config 中读取）
	// for shardID, chainCfg := range config.GlobalChain.ShardConfigs {
	// 	nh.shardChain.config[shardID] = chainCfg
	// 	// 初始化分片状态（默认上下游分片ID，领导者ID初始为0）
	// 	nh.shardChain.state[shardID] = ShardChainState{
	// 		upstreamShardID:   chainCfg.PrevShardID,
	// 		downstreamShardID: chainCfg.NextShardID,
	// 		healthy:           false, // 初始未连接
	// 	}
	// }
...
func (nh *NodeHost) chainHealthChecker() {
	ticker := time.NewTicker(nh.chain.config.DynamicAdjustInterval)
	defer ticker.Stop()
	for range ticker.C {
		nh.chain.RLock()
		for shardID, conn := range nh.chain.connections {
			if time.Since(conn.lastActive) > 2*nh.chain.config.DynamicAdjustInterval {
				nh.handleChainFailure(shardID)
			}
		}
		nh.chain.RUnlock()
	}
