...
type NodeHost struct {
	...
	// 【删除】新增领导者链条配置
	// chainConfig struct {
	// 	leaderCount   uint64 // 链中领导者数量
	// 	followerCount uint64 // 每个领导者的跟随者数量
	// }
	// leaderChain []uint64 // 领导者链条
	...
	// shardChain struct {
	// 	sync.RWMutex
	// 	config      GlobalChainConfig       // 全局链式配置
	// 	config   map[uint64]pb.ChainConfig  // 分片级链式配置（ShardID -> 上下游分片ID/角色）
	// 	state    map[uint64]ShardChainState // 分片连接状态（ShardID -> 连接健康/同步向量）
	// 	resolver ShardLeaderResolver        // 分片领导者解析器（动态获取分片当前领导者）
	// }
	// 链式复制核心配置与状态
	// chain struct {
	// 	sync.RWMutex
	// 	config      GlobalChainConfig      // 全局链式配置
	// 	shardMap    map[uint64]ChainConfig // 分片级链式配置
	// 	connections map[uint64]*chainConn  // 活跃连接池
	// 	discovery   *serviceDiscovery      // 服务发现客户端
	// }
	chain struct {
		sync.RWMutex
		config      config.GlobalChainConfig  // 修正：使用 config 包的 GlobalChainConfig
		shardMap    map[uint64]pb.ChainConfig // 修正：使用 pb.ChainConfig 类型
		connections map[uint64]*chainConn
...
func NewNodeHost(nhConfig config.NodeHostConfig) (*NodeHost, error) {
	...
	// 【新增】
	// 初始化链式复制模块
	// nh.chain.shardMap = make(map[uint64]ChainConfig)
	// nh.chain.connections = make(map[uint64]*chainConn)
	// nh.chain.config = config.GlobalChain
	...
	// 【新增】加载分片级链式配置
	// for shardID, cfg := range config.GlobalChain.ShardConfigs {
	...
	if config.GlobalChain.ServiceDiscoveryAddr != "" {
		nh.chain.discovery = newServiceDiscovery(config.GlobalChain.ServiceDiscoveryAddr)
	}
	// 启动链式健康检查协程
...
func (nh *NodeHost) updateLeaderChain(shardID uint64, m pb.Membership) {
	...
	// 如果领导者不在链条中且链条未满，则添加到链条末尾，并尝试连接前一个领导者节点
	if !contains(nh.leaderChain, leaderID) {
		if len(nh.leaderChain) < int(nh.chainConfig.leaderCount) {
			// 添加新领导者到链条
			nh.leaderChain = append(nh.leaderChain, leaderID)
			plog.Infof("新领导者 %d 已添加到链条，当前链条: %v", leaderID, nh.leaderChain)

			// 如果不是第一个领导者，则建立与前一个领导者的连接
			if len(nh.leaderChain) > 1 {
				prevLeader := nh.leaderChain[len(nh.leaderChain)-2]
				nh.connectToPrevLeader(shardID, prevLeader, leaderID)
			}
		} else {
			plog.Warningf("领导者链条已满 (%d)，无法添加新领导者 %d", nh.chainConfig.leaderCount, leaderID)
		}
	}

	// updateLeaderChain中检查链长，超过阈值时触发链拆分：
	if len(leaderChain) > int(nh.nhConfig.GlobalChain.MaxChainLength) {
		plog.Warningf("chain length %d exceeds max %d, triggering split", len(leaderChain), nh.nhConfig.GlobalChain.MaxChainLength)
		nh.splitChain(shardID, leaderChain) // 新增链拆分逻辑
	}
...
// 新增 connectToPrevLeader 建立与前一个领导者的CRAQ链式连接
...
// 	if err != nil {
// 		plog.Errorf("获取前一个领导者 %d 地址失败: %v", prevLeader, err)
// 		return
// 	}

// 	plog.Infof("建立领导者 %d 到前一个领导者 %d 的链式连接", currLeader, prevLeader)

// 	// 发送LeaderChainConnect消息给前一个领导者
// 	msg := pb.Message{
...
func (nh *NodeHost) connectToPrevLeader(shardID, prevLeader, currLeader uint64) {
	...
	// 5. 补充连接元数据（帮助前领导者验证和双向通信）
	// msg.Snapshot = pb.Snapshot{
	// 	LeaderAddress: currLeaderAddr,                // 当前领导者地址（用于反向连接）
	// 	Timestamp:     uint64(time.Now().UnixNano()), // 消息时间戳（防重放）
	// 	ChainVersion:  uint64(len(nh.leaderChain)),   // 领导者链条长度（版本控制）
	// 	ShardID:       shardID,                       // 分片ID（多分片隔离）
	// }
	// 5. 补充连接元数据（序列化 SnapshotHeader 存入 Checksum 字段）
	//由于 pb.Snapshot 结构体（代码生成文件，位于 d:/ago/src/raft-dragonboat/raftpb/snapshot.go）不支持新增字段，且其 Checksum 字段（类型 []byte）可用于存储自定义元数据，因此通过序列化 SnapshotHeader 到 Checksum 字段传递领导者链条元数据。
	// 创建元数据结构体并填充数据
	...
	// 6. 发送消息并记录调试日志
	nh.sendMessage(msg)
	plog.Debugf("链式连接消息已发送 [分片 %d]: 从 %d 到 %d, 元数据: {版本: %d, 时间戳: %d}",
		shardID, currLeader, prevLeader, header.ChainVersion, header.Timestamp) // 使用 header 变量
	// plog.Debugf("链式连接消息已发送 [分片 %d]: 从 %d 到 %d, 元数据: {版本: %d, 时间戳: %d}",
	// 	shardID, currLeader, prevLeader, msg.Snapshot.ChainVersion, msg.Snapshot.Timestamp)
}
...
func (nh *NodeHost) checkChainLength() {
	currentLen := len(nh.leaderChain)
	if currentLen < int(nh.chainConfig.leaderCount) {
		plog.Warningf("领导者链条长度不足: 当前 %d, 预期 %d", currentLen, nh.chainConfig.leaderCount)
	} else if currentLen > int(nh.chainConfig.leaderCount) {
		plog.Warningf("领导者链条长度过长: 当前 %d, 预期 %d", currentLen, nh.chainConfig.leaderCount)
	} else {
		plog.Debugf("领导者链条长度正常: %d", currentLen)
	}
}
...
func (nh *NodeHost) connectDownstream(conn *chainConn) error {
	...
	req := &pb.ChainSyncRequest{
		ShardID:    conn.shardID,
		LeaderID:   nh.getLocalReplicaID(conn.shardID), // 获取本地分片副本ID
	...
	for range ticker.C {
		...
		for shardID, conn := range nh.chain.connections {
			...
			if time.Since(conn.lastActive) > nh.chain.config.HealthCheckTimeout {
				plog.Warningf("shard %d chain connection timeout, trying to reconnect", shardID)
				nh.chain.RUnlock()
				go nh.reconnectShard(shardID)
				nh.chain.RLock()
			} else if conn.healthStatus {
				// 发送心跳维持连接
				go nh.sendChainHeartbeat(shardID)
			}
		}
		nh.chain.RUnlock()
	}
...
func (nh *NodeHost) checkShardChainHealth() {
	...
	for shardID, state := range nh.shardChain.state {
		// 检查状态同步超时（如 3 次同步周期未更新）
		if time.Since(state.lastSyncTime) > 3*nh.config.GlobalChain.SyncInterval {
			state.healthy = false
			nh.shardChain.state[shardID] = state
