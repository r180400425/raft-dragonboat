...
func (nh *NodeHost) connectToPrevLeader(shardID, prevLeader, currLeader uint64) {
	...
	header := &pb.SnapshotHeader{
		...
		// 【修正】使用分片链式配置版本替代未定义的 leaderChain
		ChainVersion: nh.chain.shardMap[shardID].Version, // 使用分片配置的版本号
	...
	plog.Debugf("链式连接消息已发送 [分片 %d]: 从 %d 到 %d, 元数据: {版本: %d, 时间戳: %d}",
		shardID, currLeader, prevLeader, header.ChainVersion, header.Timestamp)
}
