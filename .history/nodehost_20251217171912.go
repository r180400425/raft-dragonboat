...
type NodeHost struct {
	...
	// 若启用链式复制（即配置了 prevShardID、nextShardID 等链式拓扑参数），则 serviceDiscovery 是必需的。
	// 因为链式复制依赖动态获取上下游分片的领导者地址，否则无法建立跨分片的同步连接（如 connectDownstream 会因无法获取领导者地址而失败）。

	// 若未启用链式复制（即无需分片间的链式同步），则 serviceDiscovery 是可选的。
	// 此时可忽略 ServiceDiscoveryAddr 配置，相关链式连接逻辑（如 initChainConnection、connectDownstream）不会被触发。
	chain struct {
		sync.RWMutex
		config      config.GlobalChainConfig  //  全局链式配置
		shardMap    map[uint64]pb.ChainConfig // 分片级链式配置：使用 pb.ChainConfig 类型（ShardID -> 上下游分片ID/角色）
...
func NewNodeHost(nhConfig config.NodeHostConfig) (*NodeHost, error) {
	...
	nh.chain.shardMap = make(map[uint64]pb.ChainConfig)
	nh.chain.connections = make(map[uint64]*chainConn)
	nh.chain.config = nhConfig.GlobalChain // 修正：使用传入的配置而非包级变量

	// 【修正】加载分片级链式配置 - 从 nhConfig 读取分片配置
	for shardID, cfg := range nhConfig.ChainConfigs { // 新增：NodeHostConfig 应包含 ChainConfigs 字段
		nh.chain.shardMap[shardID] = cfg
	...
	if nh.chain.config.ServiceDiscoveryAddr != "" {
		nh.chain.discovery = newServiceDiscovery(nh.chain.config.ServiceDiscoveryAddr, nh)
	}
	// 启动链式健康检查协程
...
func (nh *NodeHost) updateLeaderChain(shardID uint64, m pb.Membership) {
	...
	chainCfg, exists := nh.chain.shardMap[shardID]
	if !exists {
...
func (nh *NodeHost) handleChainFailure(failedShardID uint64) error {
	...
	// 从可用分片列表选择新下游
	newNext := uint64(0)
	for _, candidate := range nh.chain.shardMap[conn.prevShardID].AvailableShards {
...
//		if ok {
//			n.send(pb.Message{
//				Type:    pb.ChainConfigUpdate,
//				ShardID: shardID,
//				Data:    nh.serializeChainConfig(shardID),
//			})
//		}
//	}
//
...
func (nh *NodeHost) notifyShardUpdate(shardID uint64) {
	n, ok := nh.getShard(shardID)
	if !ok {
		plog.Warningf("shard %d not found when trying to notify chain config update", shardID)
		return
	}

	// 从链式配置中获取分片配置
	nh.chain.RLock()
	chainConfig, exists := nh.chain.shardMap[shardID]
	nh.chain.RUnlock()

	if !exists {
		plog.Warningf("chain config for shard %d not found", shardID)
		return
	}

	// 序列化链式配置
	configData, err := proto.Marshal(&chainConfig)
	if err != nil {
		plog.Errorf("failed to serialize chain config for shard %d: %v", shardID, err)
		return
	}

	// 使用正确的消息类型和字段名
	msg := pb.Message{
		Type:    pb.MessageType_ChainConfigUpdate,
		ShardID: shardID,
		Payload: configData,
	}

	// 通过NodeHost的sendMessage方法发送消息
	nh.sendMessage(msg)
	plog.Debugf("sent chain config update notification to shard %d", shardID)
}
