...
type NodeHost struct {
	...
	chain struct {
		sync.RWMutex
		chainConfigs map[uint64]pb.ChainConfig // 分片级链式配置：使用 pb.ChainConfig 类型（ShardID -> 上下游分片ID/角色）
...
func NewNodeHost(nhConfig config.NodeHostConfig) (*NodeHost, error) {
	...
	nh.chain.chainConfigs = make(map[uint64]pb.ChainConfig)
	...
	for shardID, cfg := range nhConfig.ChainConfigs { // 新增：NodeHostConfig 应包含 ChainConfigs 字段
		nh.chain.chainConfigs[shardID] = cfg
