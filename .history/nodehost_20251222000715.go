...
import (
	...
	"github.com/lni/dragonboat/v4/internal/raft" //新增
...
type NodeHost struct {
	...
	env        *server.Env
	engine     *engine
	nhConfig   config.NodeHostConfig //中已经有chainConfigs配置

	requestPools []*sync.Pool
	partitioned  int32
	closed       int32
	// 一、结构体定义：新增分片级链式配置与状态（NodeHost 结构体内部）
	// 需在 NodeHost 结构体中新增分片级链式状态管理字段，替换原基于领导者的 chainConfig 和 leaderChain。
	// 【新增】分片级链式配置与状态
	// 一、serviceDiscovery 的核心作用
	// 动态发现分片领导者地址
	// 在链式复制中，每个分片（Shard）需要与上下游分片建立连接（如 prevShardID 和 nextShardID）。由于 Raft 分片的领导者（Leader）可能因故障转移而变化，serviceDiscovery 用于动态获取目标分片的当前领导者地址，确保链式连接的正确性。

	// 例如，在 connectDownstream 方法中，通过 nh.chain.discovery.GetShardLeader(conn.nextShardID) 获取下游分片的领导者地址，进而建立同步连接。
	// 支撑分片级链式拓扑的动态维护
	// 当分片的领导者发生变更时（如主节点故障、重新选举），服务发现可实时更新领导者信息，避免因静态配置导致的连接失效，保证链式复制的连续性。

	chain struct {
		sync.RWMutex
		connections map[uint64]*chainConn // 分片连接状态（ShardID -> 连接健康/同步向量）
		discovery   *serviceDiscovery     // 服务发现客户端
	}
}

// 【新增】链式连接状态
// chainConn 链式复制的分片连接状态，维护与上下游分片的连接信息和同步状态
type chainConn struct {
	shardID     uint64    // 当前分片ID
	prevShardID uint64    // 上游分片ID（0表示无上游）
	nextShardID uint64    // 下游分片ID（0表示无下游）
	leaderAddr  string    // 目标分片领导者地址
	lastActive  time.Time // 最后活动时间（用于连接超时检测）
	// 新增：最后同步时间戳（用于健康检查）
	lastSyncTimestamp uint64                    // 最后同步操作的时间戳（毫秒级Unix时间）
	healthStatus      bool                      // 连接健康状态标识
	syncChan          chan *pb.ChainSyncRequest // 链式同步请求通道（缓冲大小100）
	// 新增：同步索引。初始化同步索引为0，表示同步索引未初始化
	syncIndex uint64 // 已同步至下游的日志索引位置
}

// 【新增】服务发现客户端结构体定义
type serviceDiscovery struct {
	addr string    // 服务发现服务器地址
	nh   *NodeHost // 添加 NodeHost 引用，用于访问集群信息
}

// 【新增】服务发现客户端构造函数
func newServiceDiscovery(addr string, nh *NodeHost) *serviceDiscovery {
	return &serviceDiscovery{
		addr: addr,
		nh:   nh, // 保存 NodeHost 引用
	}
}
...
func NewNodeHost(nhConfig config.NodeHostConfig) (*NodeHost, error) {
	...
	// 二、初始化逻辑：分片链式状态初始化（NewNodeHost 方法）
	// 需在 NodeHost 初始化时，初始化分片级链式配置的存储结构，并加载默认分片链式配置（如从配置文件或启动参数）。

	// 【修正】初始化链式复制模块 - 从 nhConfig 加载全局配置
	nh.chain.connections = make(map[uint64]*chainConn)

	// 【修正】加载分片级链式配置 - 从 nhConfig 读取分片配置
	if nhConfig.ChainConfigs != nil {
		for shardID, cfg := range nhConfig.ChainConfigs {
			nh.chain.connections[shardID] = &chainConn{
				shardID:      shardID,
				prevShardID:  cfg.PrevShardId,
				nextShardID:  cfg.NextShardId,
				syncChan:     make(chan *pb.ChainSyncRequest, 100),
				healthStatus: false,
				syncIndex:    0, // 新增：初始化同步索引为0
			}
		}
	}

	// 【修正】使用实例配置的服务发现地址而非全局配置
	// 【修改】初始化服务发现客户端时传入 NodeHost 实例
	if nh.nhConfig.ChainConfigs.ServiceDiscoveryAddr != "" {
		nh.chain.discovery = newServiceDiscovery(nh.nhConfig.ChainConfigs.ServiceDiscoveryAddr, nh)
	}
	// 启动链式健康检查协程
	go nh.chainHealthChecker()
...
// 作用：这是 NodeHost 对外暴露的成员信息结构体（如通过 SyncGetShardMembership 方法返回），字段与 raftpb.Membership 基本对应，但 Removed 字段类型从 map[uint64]bool 改为 map[uint64]struct{}（更节省内存的空集合表示），Addresses 重命名为 Nodes（更直观的命名）。
...
func (nh *NodeHost) SyncGetShardMembership(ctx context.Context,
	...
	v, err := nh.linearizableRead(ctx, shardID,
		func(node *node) (interface{}, error) {
			...
			//新增  获取当前集群成员后，更新领导者链条
			go nh.updateLeaderChain(shardID, m)
...
// 新增 updateLeaderChain 更新领导者链条并检查长度是否正常
func (nh *NodeHost) updateLeaderChain(shardID uint64, m pb.Membership) {
	// 修复：使用 chain 结构体中的锁替代已删除的 chainMu
	nh.chain.Lock()
	defer nh.chain.Unlock()

	// 获取当前领导者
	leaderID, _, _, err := nh.GetLeaderID(shardID)
	// 调整：使用 raft.InvalidReplicaID 替代未定义的 raft.None
	if err != nil || leaderID == raft.NoLeader { // raft.None 需要根据实际代码调整
		plog.Warningf("无法获取分片 %d 的领导者", shardID)
		return
	}

	// 修复：使用分片配置替代已删除的 leaderChain
	chainCfg, exists := nh.chain.chainConfigs[shardID]
	if !exists {
		plog.Warningf("分片 %d 的链式配置不存在", shardID)
		return
	}

	// 检查当前分片是否为链条中的领导者节点
	if chainCfg.ChainRole == pb.ChainRole_HEAD {
		// 检查链条长度是否超过全局配置限制
		if nh.chain.config.MaxChainLength > 0 && len(nh.chain.connections) > int(nh.chain.config.MaxChainLength) {
			plog.Warningf("链条长度 %d 超过最大限制 %d，触发拆分",
				len(nh.chain.connections), nh.chain.config.MaxChainLength)
			// 【修正】注释掉未实现的 splitChain 调用，待实现后取消注释
			// nh.splitChain(shardID)
		}
	}

	// 检查链条健康状态
	nh.checkShardChainHealth()
}

// connectToPrevLeader 建立当前领导者与前领导者的链式连接
func (nh *NodeHost) connectToPrevLeader(shardID, prevLeader, currLeader uint64) {
	// 1. 输入参数基础验证
	if shardID == 0 || prevLeader == 0 || currLeader == 0 {
		plog.Errorf("无效参数，分片ID: %d, 前领导者ID: %d, 当前领导者ID: %d", shardID, prevLeader, currLeader)
		return
	}

	// 2. 验证前领导者是否为当前集群有效成员
	if !nh.isReplicaInMembers(shardID, prevLeader) {
		plog.Errorf("前领导者 %d 已不在分片 %d 的成员列表中，无法建立连接", prevLeader, shardID)
		return
	}

	// 3. 获取前/当前领导者地址
	prevLeaderAddr, err := nh.getNodeAddress(shardID, prevLeader)
	if err != nil {
		plog.Errorf("获取前领导者 %d (分片 %d) 地址失败: %v", prevLeader, shardID, err)
		return
	}
	if prevLeaderAddr == "" {
		plog.Errorf("前领导者 %d (分片 %d) 的地址为空", prevLeader, shardID)
		return
	}

	currLeaderAddr, err := nh.getNodeAddress(shardID, currLeader)
	if err != nil {
		plog.Errorf("获取当前领导者 %d (分片 %d) 地址失败: %v", currLeader, shardID, err)
		return
	}
	if currLeaderAddr == "" {
		plog.Errorf("当前领导者 %d (分片 %d) 的地址为空", currLeader, shardID)
		return
	}

	// 4. 构建链式连接消息（含元数据）
	plog.Infof("建立领导者链式连接 [分片 %d]: 当前领导者 %d (%s) -> 前领导者 %d (%s)",
		shardID, currLeader, currLeaderAddr, prevLeader, prevLeaderAddr)

	msg := pb.Message{
		Type:    pb.LeaderChainConnect,
		ShardID: shardID,
		From:    currLeader,
		To:      prevLeader,
	}

	// 5. 补充连接元数据（序列化 SnapshotHeader 存入 Checksum 字段）
	// 解决 pb.Snapshot 结构体无法新增字段的限制，通过 Checksum 传递自定义元数据
	header := &pb.SnapshotHeader{
		LeaderAddress: currLeaderAddr,                // 当前领导者地址（用于反向连接）
		Timestamp:     uint64(time.Now().UnixNano()), // 消息时间戳（防重放）
		ShardID:       shardID,                       // 分片ID（多分片隔离）
	}
	// 序列化元数据（使用 SnapshotHeader 自带的 Marshal 方法）
	headerData, err := header.Marshal()
	if err != nil {
		plog.Errorf("序列化领导者链条元数据失败: %v", err)
		return
	}
	// 填充 Snapshot 结构体，使用 Checksum 字段存储元数据
	msg.Snapshot = pb.Snapshot{
		Checksum: headerData, // 存储序列化后的元数据
		ShardID:  shardID,    // 直接设置分片ID（Snapshot 原生字段）
	}

	// 6. 发送消息并记录调试日志
	nh.sendMessage(msg)
	plog.Debugf("链式连接消息已发送 [分片 %d]: 从 %d 到 %d, 元数据: {时间戳: %d}",
		shardID, currLeader, prevLeader, header.Timestamp)
}

// 新增辅助函数：检查副本是否在当前集群成员列表中
func (nh *NodeHost) isReplicaInMembers(shardID, replicaID uint64) bool {
	nh.mu.RLock()
	defer nh.mu.RUnlock()

	node, ok := nh.getShard(shardID)
	if !ok {
		return false
	}
	// 从状态机获取最新成员信息（修正：替换 node.members 为 node.sm.GetMembership()）
	m := node.sm.GetMembership()
	// 检查是否为投票成员、非投票成员或见证者
	_, isVoting := m.Addresses[replicaID]
	_, isNonVoting := m.NonVotings[replicaID]
	_, isWitness := m.Witnesses[replicaID]
	return isVoting || isNonVoting || isWitness
}

// 新增 checkChainLength 检查链条长度是否正常并发出提醒
func (nh *NodeHost) checkChainLength() {
	// 使用分片连接映射长度作为当前链条长度，加读锁保证线程安全
	nh.chain.RLock()
	currentLen := len(nh.chain.connections)
	nh.chain.RUnlock()

	// 从分片级链式配置获取最大允许长度
	maxLen := int(nh.chain.config.MaxChainLength)
	if currentLen < maxLen {
		plog.Warningf("分片链式连接数量不足: 当前 %d, 最大允许 %d", currentLen, maxLen)
	} else if currentLen > maxLen {
		plog.Warningf("分片链式连接数量超限: 当前 %d, 最大允许 %d", currentLen, maxLen)
	} else {
		plog.Debugf("分片链式连接数量正常: %d", currentLen)
	}
}
...
func (nh *NodeHost) getNodeAddress(shardID, replicaID uint64) (string, error) {
	nh.mu.RLock()
	defer nh.mu.RUnlock()
	...
	// 从节点成员信息中查找地址
	// for id, addr := range n.members.Addresses {
	// 	if id == replicaID {
	// 		return addr, nil
	// 	}
	// }

	// 从节点成员信息中查找地址（依次检查投票成员、非投票成员、见证者）
	// 从状态机获取最新成员信息（修正：替换 n.members 为 n.sm.GetMembership()）
	m := n.sm.GetMembership()

	// 从节点成员信息中查找地址（依次检查投票成员、非投票成员、见证者）
	// 1. 检查投票成员
	if addr, ok := m.Addresses[replicaID]; ok {
		return addr, nil
	}
	// 2. 检查非投票成员
	if addr, ok := m.NonVotings[replicaID]; ok {
		return addr, nil
	}
	// 3. 检查见证者节点
	if addr, ok := m.Witnesses[replicaID]; ok {
		return addr, nil
	}

	return "", fmt.Errorf("节点 %d 地址未找到", replicaID)
}
...
func (nh *NodeHost) startShard(initialMembers map[uint64]Target,
	...
	doStart := func() (*node, error) {
		...
		// 【新增】分片启动后初始化链式连接
		if conn, ok := nh.chain.connections[shardID]; ok {
			go nh.initChainConnection(conn)
		}
		return rn, nil
	}
...
func (nh *NodeHost) createTransport() error {
	...
	// 修改前：nh.nodes 直接作为 registry.IResolver 参数传递
	// 修改后：通过类型断言将 nh.nodes 转换为 registry.IResolver
	// 安全转换为 registry.IResolver
	resolver, ok := nh.nodes.(registry.IResolver)
	if !ok {
		return errors.Errorf("node registry does not implement registry.IResolver")
	}

	tsp, err := transport.NewTransport(nh.nhConfig,
		nh.msgHandler, nh.env, resolver, getSnapshotDir,
		&transportEvent{nh: nh}, nh.fs)
	// tsp, err := transport.NewTransport(nh.nhConfig,
	// 	nh.msgHandler, nh.env, nh.nodes.(registry.IResolver), getSnapshotDir,
	// 	&transportEvent{nh: nh}, nh.fs)
	if err != nil {
		return err
	}
...
func (h *messageHandler) HandleMessageBatch(msg pb.MessageBatch) (uint64, uint64) {
	...
	if nh.isPartitioned() {
		...
		for _, req := range msg.Requests {
			...
			// 新增：保留链式连接消息
			if req.Type >= pb.LeaderChainConnect && req.Type <= pb.LeaderChainDisconnect {
				keep = true
			}
		}
		if !keep {
	...
	for _, req := range msg.Requests {
		...
		if n, ok := nh.getShard(req.ShardID); ok {
			...
			case pb.SnapshotReceived:
				...
				msgCount++
			case pb.LeaderChainConnect, pb.LeaderChainAck,
				pb.LeaderChainPing, pb.LeaderChainDisconnect:
				// 链式连接协议消息处理
				if added, stopped := n.mq.Add(req); !added || stopped {
					plog.Warningf("dropped chain message %s for %s",
						req.Type, dn(req.ShardID, req.To))
				} else {
					msgCount++
				}
			default:
				// 处理其他所有消息类型
				if added, stopped := n.mq.Add(req); !added || stopped {
					plog.Warningf("dropped incoming %s message for %s",
						req.Type, dn(req.ShardID, req.To))
				} else {
					msgCount++
				}
...
// 新增
// 【新增】初始化分片链式连接
func (nh *NodeHost) initChainConnection(conn *chainConn) {
	shardID := conn.shardID
	// 1. 连接下游分片
	if conn.nextShardID > 0 {
		if err := nh.connectDownstream(conn); err != nil {
			plog.Errorf("shard %d connect downstream %d failed: %v", shardID, conn.nextShardID, err)
		} else {
			plog.Infof("shard %d connected to downstream %d", shardID, conn.nextShardID)
		}
	}
	// 2. 通知上游分片
	if conn.prevShardID > 0 {
		if err := nh.notifyUpstream(conn); err != nil {
			plog.Errorf("shard %d notify upstream %d failed: %v", shardID, conn.prevShardID, err)
		}
	}
}
...
// 【新增】通知上游分片
func (nh *NodeHost) notifyUpstream(conn *chainConn) error {
	// 验证上游分片ID有效性
	if conn.prevShardID == 0 {
		return errors.New("upstream shard ID cannot be 0")
	}

	// 通过服务发现获取上游分片领导者地址
	leaderAddr, err := nh.chain.discovery.GetShardLeader(conn.prevShardID)
	if err != nil {
		return errors.Wrapf(err, "get upstream %d leader", conn.prevShardID)
	}
	// 检查领导者地址不为空
	if leaderAddr == "" {
		return errors.Errorf("empty leader address for upstream shard %d", conn.prevShardID)
	}

	// 获取本地分片副本ID
	localReplicaID := nh.getLocalReplicaID(conn.shardID)
	if localReplicaID == 0 {
		return errors.Errorf("invalid local replica ID for shard %d", conn.shardID)
	}
	// 新增：获取当前分片最新日志索引
	lastLogIndex := nh.getShardLastLogIndex(conn.shardID)
	// 发送上游通知请求
	req := &pb.ChainSyncRequest{
		ShardId:    conn.shardID,
		LeaderId:   localReplicaID,
		LeaderAddr: nh.RaftAddress(),
		Timestamp:  uint64(time.Now().UnixMilli()),
		SyncIndex:  conn.syncIndex,
		// 新增：添加日志同步范围
		FromIndex: conn.syncIndex + 1, // 上次同步索引+1
		ToIndex:   lastLogIndex,       // 当前最新日志索引
	}

	// 通过transport发送请求

	// err = nh.transport.SendMessageToAddress(leaderAddr, &pb.Message{
	// 	Type:      pb.ChainSyncRequest,
	// 	ShardID:   conn.prevShardID,
	// 	From:      req.LeaderId,
	// 	To:        0, // 由目标节点路由到领导者
	// 	ChainSync: req,
	// })
	// if err != nil {
	// 	return errors.Wrapf(err, "send upstream notify to %s", leaderAddr)
	// }

	// 构造并发送消息
	msg := pb.Message{
		Type:      pb.LeaderChainConnect, // 修复：使用正确的MessageType而不是类型名// 使用链式连接专用消息类型
		ShardID:   conn.nextShardID,
		From:      localReplicaID,
		To:        0, // 由目标节点路由到领导者
		ChainSync: req,
	}
	nh.sendMessage(msg) // 使用现有发送方法
	// 更新连接状态
	nh.chain.Lock()
	defer nh.chain.Unlock()
	// 注意：这里我们不更新leaderAddr，因为这是向上游发送通知，而不是连接到下游
	conn.leaderAddr = leaderAddr // 保留地址缓存用于健康检查
	conn.lastActive = time.Now()
	conn.healthStatus = true
	conn.lastSyncTimestamp = req.Timestamp // 记录最后同步时间戳
	// 新增：记录同步索引
	conn.syncIndex = lastLogIndex
	return nil
}
...
// 【新增】连接下游分片
func (nh *NodeHost) connectDownstream(conn *chainConn) error {
	// 验证下游分片ID有效性
	if conn.nextShardID == 0 {
		return errors.New("downstream shard ID cannot be 0")
	}

	// 通过服务发现获取下游分片领导者地址
	leaderAddr, err := nh.chain.discovery.GetShardLeader(conn.nextShardID)
	if err != nil {
		return errors.Wrapf(err, "get downstream %d leader", conn.nextShardID)
	}
	// 检查领导者地址不为空
	if leaderAddr == "" {
		return errors.Errorf("empty leader address for downstream shard %d", conn.nextShardID)
	}
	// 获取本地分片副本ID并验证有效性
	localReplicaID := nh.getLocalReplicaID(conn.shardID)
	if localReplicaID == 0 {
	...
	// })
	// if err != nil {
	// 	return errors.Wrapf(err, "send chain sync request to %s", leaderAddr)
	// }
	msg := pb.Message{
		Type:      pb.LeaderChainConnect, // 使用链式连接专用消息类型
		ShardID:   conn.nextShardID,
		From:      localReplicaID,
		To:        0, // 由目标节点路由到领导者
		ChainSync: req,
	...
	// 更新连接状态（使用defer确保锁释放）
	//锁机制优化：使用defer确保锁释放，避免潜在的死锁风险
	nh.chain.Lock()
	defer nh.chain.Unlock()
	conn.leaderAddr = leaderAddr
	conn.lastActive = time.Now()
	conn.healthStatus = true
	//新增lastSyncTimestamp记录最后同步时间戳，辅助健康检查
	conn.lastSyncTimestamp = req.Timestamp // 记录最后同步时间戳
	// 新增：记录同步索引
	conn.syncIndex = lastLogIndex
	return nil
...
// 【新增】实现 GetShardLeader 方法，查询指定分片的领导者地址
func (sd *serviceDiscovery) GetShardLeader(shardID uint64) (string, error) {
	// 1. 获取分片当前领导者 ID
	leaderID, _, _, err := sd.nh.GetLeaderID(shardID)
	if err != nil {
		return "", errors.Wrapf(err, "获取分片 %d 领导者失败", shardID)
	}
	if leaderID == raft.NoLeader {
		return "", errors.Errorf("分片 %d 暂无领导者", shardID)
	}

	// 2. 查询领导者节点地址
	addr, err := sd.nh.getNodeAddress(shardID, leaderID)
	if err != nil {
		return "", errors.Wrapf(err, "获取领导者 %d 地址失败", leaderID)
	}
	return addr, nil
}
...
// 【新增】获取本地分片副本ID
func (nh *NodeHost) getLocalReplicaID(shardID uint64) uint64 {
	n, ok := nh.getShard(shardID)
	if !ok {
		plog.Errorf("shard %d not found when getting local replica ID", shardID)
		return 0
	}
	return n.replicaID
}
...
// 【新增】3. 添加链式复制核心方法
// 链式健康检查协程
func (nh *NodeHost) chainHealthChecker() {
	// 使用 作为健康检查间隔
	checkInterval := time.Duration(nh.chain.config.SyncIntervalMs) * time.Millisecond
	...
	for {
		...
		case <-ticker.C:
			nh.chain.RLock()
			connections := make([]*chainConn, 0, len(nh.chain.connections))
			for _, conn := range nh.chain.connections {
			...
			for _, conn := range connections {
				...
				// 检查连接超时（使用 SyncInterval 的 3 倍作为超时阈值）
				if time.Since(conn.lastActive) > 3*checkInterval {
					plog.Warningf("分片 %d 链式连接超时，尝试重连", conn.shardID)
					go nh.reconnectShard(conn.shardID)
				} else if conn.healthStatus {
					// 发送健康检查心跳
					go nh.sendChainHeartbeat(conn.shardID)
				}
			}
		case <-nh.stopper.ShouldStop():
			return
...
// 处理链式故障转移
func (nh *NodeHost) handleChainFailure(failedShardID uint64) error {
	nh.chain.Lock()
	defer nh.chain.Unlock()
	conn, ok := nh.chain.connections[failedShardID]
	if !ok {
		return errors.New("shard not in chain")
	}
	// 从可用分片列表选择新下游
	newNext := uint64(0)
	for _, candidate := range nh.chain.chainConfigs[conn.prevShardID].AvailableShards {
		if candidate != failedShardID && nh.checkShardHealth(candidate) {
			newNext = candidate
			break
		}
	}
	if newNext == 0 {
		return errors.New("no available fallback shard")
	}
	// 更新上游分片的下游连接
	nh.chain.connections[conn.prevShardID].nextShardID = newNext
	nh.notifyShardUpdate(conn.prevShardID)
	return nil
}

// 新增：检查分片健康状态的方法
func (nh *NodeHost) checkShardHealth(shardID uint64) bool {
	// 检查分片是否存在且正在运行
	n, ok := nh.getShard(shardID)
	if !ok {
		return false
	...
	// 检查分片是否有领导者
	leaderID, _, ready := n.getLeaderID()
	if !ready || leaderID == 0 {
		return false
	}

	// 检查链式连接状态（如果存在）
	if conn, exists := nh.chain.connections[shardID]; exists {
		// 如果有链式连接，检查连接是否健康
		if time.Since(conn.lastActive) > time.Duration(nh.chain.config.SyncIntervalMs)*time.Millisecond*3 {
			return false
		}
		return conn.healthStatus
	}
...
// 通知分片更新链式配置
func (nh *NodeHost) notifyShardUpdate(shardID uint64) {
	n, ok := nh.getShard(shardID)
	if ok {
		// 使用现有的消息发送方法替代 n.send
		nh.sendMessage(pb.Message{
			Type:    pb.LeaderChainConnect, // 使用现有的链式连接消息类型
			ShardID: shardID,
			From:    n.replicaID,
			To:      n.replicaID, // 发送给自己
			// 使用 ChainSync 字段传递配置信息
			ChainSync: &pb.ChainSyncRequest{
				ShardId: shardID,
				// 可以在这里添加配置更新相关的字段
				LeaderId: n.replicaID,
			},
		})
	}
}

// 三、状态更新方法：分片领导者变更与链式状态维护（事件处理/领导者变更回调）
// 需在分片领导者变更时，更新对应分片的 upstreamLeaderID 或 downstreamLeaderID，但保持 upstreamShardID 和 downstreamShardID 不变（实现分片级连接稳定性）。

// 修改位置：nodehost.go 中领导者变更事件处理处

// 【新增】处理分片领导者变更事件
func (nh *NodeHost) updateShardLeader(shardID uint64, newLeaderID uint64) {
	nh.shardChain.Lock()
	defer nh.shardChain.Unlock()

	// 获取分片链式状态
	shardState, ok := nh.shardChain.state[shardID]
	if !ok {
		plog.Warningf("shard %d not found in chain state", shardID)
		return
	}

	// 更新当前分片的领导者ID（若自身是其他分片的上下游）
	for targetShardID, targetState := range nh.shardChain.state {
		// 若目标分片以上游分片ID为当前分片ID，则更新其上游领导者ID
		if targetState.upstreamShardID == shardID {
			nh.shardChain.state[targetShardID].upstreamLeaderID = newLeaderID
			plog.Debugf("shard %d upstream leader updated to %d", targetShardID, newLeaderID)
		}
		// 若目标分片以下游分片ID为当前分片ID，则更新其下游领导者ID
		if targetState.downstreamShardID == shardID {
			nh.shardChain.state[targetShardID].downstreamLeaderID = newLeaderID
			plog.Debugf("shard %d downstream leader updated to %d", targetShardID, newLeaderID)
		}
	}

	// 标记分片连接为健康（触发状态同步）
	shardState.healthy = true
	shardState.lastSyncTime = time.Now()
	nh.shardChain.state[shardID] = shardState
}

/*
四、辅助方法：分片状态向量同步与健康检查（新增方法）
需新增分片状态向量同步和健康检查方法，确保分片间轻量级一致性协议的实现（如定期交换状态向量、检测连接健康）。

修改位置：nodehost.go 中新增 syncShardStateVectors 和 checkShardChainHealth 方法。
*/

// 【新增】分片状态向量同步（定期执行）
func (nh *NodeHost) syncShardStateVectors() {
	nh.shardChain.RLock()
	defer nh.shardChain.RUnlock()

	for shardID, state := range nh.shardChain.state {
		// 跳过未初始化或不健康的分片
		if !state.healthy || state.downstreamShardID == 0 {
			continue
		}

		// 获取下游分片当前领导者
		downstreamLeader, err := nh.shardChain.resolver.ResolveLeader(state.downstreamShardID)
		if err != nil {
			plog.Warningf("shard %d resolve downstream leader failed: %v", shardID, err)
			continue
		}

		// 构造状态向量消息（包含当前分片的日志索引、同步状态等）
		vec := pb.ShardStateVector{
			ShardID:       shardID,
			LastLogIndex:  nh.getShardLastLogIndex(shardID), // 需实现：获取分片最新日志索引
			UpstreamAck:   state.upstreamLeaderID,
			DownstreamAck: state.downstreamLeaderID,
			Timestamp:     time.Now().UnixMilli(),
		}

		// 发送状态向量到下游分片领导者（通过 transport）
		nh.transport.SendMessageToShardLeader(
			state.downstreamShardID,
			pb.Message{
				Type:        pb.ShardStateVector,
				From:        nh.id.GetReplicaID(),
				To:          downstreamLeader,
				StateVector: vec,
			},
		)
...
// 【修正】获取分片最新日志索引（删除重复函数，修复GetRange调用）
func (nh *NodeHost) getShardLastLogIndex(shardID uint64) uint64 {
	n, ok := nh.getShard(shardID)
	if !ok {
		plog.Errorf("getShardLastLogIndex: shard %d not found", shardID)
		return 0
	}
	// 通过日志读取器获取日志范围 [first, last]
	// 修正：GetRange() 仅返回两个值 (first, last)，无错误返回
	_, lastIndex := n.logReader.GetRange()
	return lastIndex
}

// 【新增】分片链式健康检查（定期执行）
func (nh *NodeHost) checkShardChainHealth() {
	nh.chain.Lock()
	defer nh.chain.Unlock()

	// 遍历所有分片的链式连接状态
	for shardID, conn := range nh.chain.connections {
		// 检查状态同步超时（3倍同步周期未活动）
		if time.Since(conn.lastActive) > 3*nh.nhConfig.GlobalChain.SyncInterval {
			conn.healthStatus = false
			plog.Warningf("shard %d chain state sync timeout, mark as unhealthy", shardID)
			// 触发重连逻辑（异步调用连接方法）
			go nh.reconnectShard(shardID)
		}
	}
}

// 【新增】发送链式心跳
func (nh *NodeHost) sendChainHeartbeat(shardID uint64) {
	// 实现发送心跳消息的逻辑
	// ...
}

// 【新增】重连分片
func (nh *NodeHost) reconnectShard(shardID uint64) {
	nh.chain.RLock()
	conn, ok := nh.chain.connections[shardID]
	nh.chain.RUnlock()
	if ok {
		conn.healthStatus = false
		nh.initChainConnection(conn)
	}
}
...
// 【修正】配置与状态管理分离
func (nh *NodeHost) initChainReplication() {
	// 1. 初始化配置管理器（低频操作）
	nh.chain.configManager = newChainConfigManager(nh)

	// 2. 从配置加载初始拓扑
	if err := nh.chain.configManager.LoadFromConfig(nh.nhConfig.ChainConfigs); err != nil {
		// 处理错误
	}

	// 3. 初始化状态管理器（高频操作）
	nh.chain.stateManager = newStateManager(nh)

	// 4. 启动状态同步协程（高频）
	go nh.chain.stateManager.startSyncWorker()

	// 5. 启动心跳协程（高频）
	go nh.chain.stateManager.startHeartbeatWorker()
}

// 配置变更处理（低频）
func (nh *NodeHost) UpdateChainTopology(shardID uint64, newConfig *config.ChainConfig) error {
	return nh.chain.configManager.Update(shardID, newConfig)
}

// 状态更新处理（高频）
func (sm *stateManager) handleHeartbeat(heartbeat *pb.ChainHeartbeat) {
	// 仅更新必要的运行时状态
	sm.updateState(heartbeat.ShardID, heartbeat.SyncIndex, heartbeat.HealthStatus)
}
...
type configCache struct {
	mu     sync.RWMutex
	config map[uint64]*ChainConfig
}

func (c *configCache) Get(shardID uint64) *ChainConfig {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
// 仅当配置实际变更时才更新缓存
func (c *configCache) Update(shardID uint64, newConfig *ChainConfig) {
	c.mu.Lock()
	defer c.mu.Unlock()

	// 深度比较，仅当真正变化时才更新
	if !reflect.DeepEqual(c.config[shardID], newConfig) {
		c.config[shardID] = newConfig
		// 触发配置变更事件
		c.notifyChange(shardID, newConfig)
	}
}
func (c *configCache) Update(shardID uint64, newConfig *ChainConfig, expectedVersion uint64) error {
	c.mu.Lock()
	defer c.mu.Unlock()

	current, exists := c.config[shardID]
	...
	newConfig.LastUpdatedBy = c.nh.id.String()
...
// 状态压缩：只发送变化的部分
func (sm *stateManager) sendHeartbeat() {
	for shardID, conn := range sm.connections {
		// 仅当状态有变化时才发送
		if sm.stateChanged(shardID) {
			heartbeat := &pb.ChainHeartbeat{
				ShardID:    shardID,
				SyncIndex:  conn.State.SyncIndex,
				LeaderTerm: conn.State.LeaderTerm,
				// 仅发送必要的状态
			}
			sm.transport.SendHeartbeat(heartbeat)
		}
	}
}

// 增量同步：只同步变化的日志
func (sm *stateManager) syncShard(shardID uint64) {
	conn := sm.getConnection(shardID)

	// 计算需要同步的日志范围
	fromIndex := conn.State.SyncIndex + 1
	toIndex := sm.getLatestLogIndex(shardID)

	if fromIndex <= toIndex {
		// 仅同步增量部分
		request := &pb.ChainSyncRequest{
			ShardID:   shardID,
			FromIndex: fromIndex,
			ToIndex:   toIndex,
			// ...
		}
		sm.transport.SendSyncRequest(request)
	}
}

// 配置变更时的安全过渡
func (cm *configManager) Update(shardID uint64, newConfig *ChainConfig) error {
	// 1. 验证新配置的有效性
	...
	// 2. 通知状态管理器准备拓扑变更
	if err := cm.stateManager.prepareTopologyChange(shardID, newConfig); err != nil {
		return err
	}

	// 3. 原子更新配置
	if err := cm.storage.Update(shardID, newConfig); err != nil {
		// 4. 如果更新失败，通知状态管理器回滚准备
		cm.stateManager.rollbackTopologyChange(shardID)
		return err
	}

	// 5. 通知状态管理器完成拓扑变更
	cm.stateManager.completeTopologyChange(shardID, newConfig)
...
// 状态管理器处理拓扑变更
// 【优化】状态管理器的拓扑变更处理
func (sm *stateManager) prepareTopologyChange(
	shardID uint64,
	newConfig *ChainConfig,
	reason string) error {

	conn, exists := sm.connections[shardID]
	if !exists {
		return errors.New("shard not found")
	}

	// 1. 检查拓扑是否形成环
	if err := sm.checkTopologyCycle(newConfig); err != nil {
		return err
	}

	// 2. 更新拓扑状态为PREPARING
	conn.State.TopologyState = TopologyStatePreparing

	// 3. 准备建立新连接（但不关闭旧连接）
	if newConfig.PrevShardId != conn.Config.PrevShardId {
		if err := sm.prepareUpstreamConnection(shardID, newConfig.PrevShardId); err != nil {
			return err
		}
	}
	if newConfig.NextShardId != conn.Config.NextShardId {
		if err := sm.prepareDownstreamConnection(shardID, newConfig.NextShardId); err != nil {
			return err
		}
	}

	plog.Infof("shard %d topology change prepared: %s -> %s (reason: %s)",
		shardID, describeTopology(conn.Config), describeTopology(newConfig), reason)

	return nil
}

func (sm *stateManager) completeTopologyChange(shardID uint64) {
	conn := sm.connections[shardID]

	// 1. 关闭旧连接
	if conn.prevConn != nil {
		conn.prevConn.Close()
		conn.prevConn = nil
	}
	if conn.nextConn != nil {
		conn.nextConn.Close()
		conn.nextConn = nil
	}

	// 2. 激活新连接
	conn.State.TopologyState = TopologyStateStable

	plog.Infof("shard %d topology change completed", shardID)
}

// 在拓扑变更时，先建立新连接再断开旧连接
// 【优化】配置变更的安全过渡
func (cm *configManager) UpdateTopology(
	shardID uint64,
	newConfig *ChainConfig,
	reason string) error {
	...
	// 2. 获取当前配置
	currentConfig := cm.cache.Get(shardID)
	...
	// 3. 尝试更新缓存（乐观锁）
	if err := cm.cache.Update(shardID, newConfig, expectedVersion); err != nil {
		return err
	}

	// 4. 通知状态管理器准备变更
	if err := cm.nh.chain.stateManager.prepareTopologyChange(shardID, newConfig, reason); err != nil {
		// 5. 如果准备失败，回滚缓存
		if currentConfig != nil {
			cm.cache.Update(shardID, currentConfig, newConfig.Version)
		}
		return err
	}

	// 6. 持久化配置
	if err := cm.storage.Save(shardID, newConfig); err != nil {
		// 7. 如果持久化失败，回滚并清理
		cm.nh.chain.stateManager.rollbackTopologyChange(shardID)
		if currentConfig != nil {
			cm.cache.Update(shardID, currentConfig, newConfig.Version)
		}
		return err
	...
	// 8. 通知状态管理器完成变更
	cm.nh.chain.stateManager.completeTopologyChange(shardID)

	return nil
}

// 对高频变化的状态进行更精细的压缩
func (sm *stateManager) compressState(state *ChainState) []byte {
	// 只编码变化的部分
	// 使用更紧凑的编码方式
	// ...
}
...
// 支持不重启更新配置
func (nh *NodeHost) ReloadConfig() error {
	// 从配置源重新加载
	newConfigs, err := loadConfigFromSource()
	if err != nil {
		return err
	}

	// 逐个分片更新配置
	for shardID, newConfig := range newConfigs {
		if err := nh.UpdateChainTopology(shardID, newConfig); err != nil {
			return err
		}
	}
...
// 在应用新配置前进行验证
func (cm *configManager) validateConfig(newConfig *ChainConfig) error {
	// 检查拓扑是否形成环
	if err := cm.checkForCycles(newConfig); err != nil {
		return err
	}

	// 检查节点可用性
	if !cm.isNodeAvailable(newConfig.PrevShardId) {
		return errors.New("upstream node not available")
	}

	// 检查角色与拓扑一致性
	if newConfig.ChainRole == pb.ChainRole_HEAD && newConfig.PrevShardId != 0 {
		return errors.New("HEAD role must have no upstream")
	}
