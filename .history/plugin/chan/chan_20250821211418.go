...
// 新增：实现 raftio.ITransport 接口的 SendMessageToShardLeader 方法
func (ct *ChanTransport) SendMessageToShardLeader(targetShardID uint64, msg pb.Message) error {
	// 测试场景下：假设目标 Shard 领导者地址为 "shard-<targetShardID>"（可根据测试需求调整）
	// 因为是测试，所以可以这样生成
	leaderAddr := fmt.Sprintf("shard-%d", targetShardID)

	// 1. 获取与目标 Shard 领导者的连接（复用现有 channel 连接机制）
	conn, err := ct.GetConnection(context.Background(), leaderAddr)
	if err != nil {
		return errors.Wrapf(err, "failed to connect to shard %d leader", targetShardID)
	}
	defer conn.Close()
