...
// 新增
// ShardLeaderResolver 定义 Shard 领导者查询接口，用于解析目标 Shard 的当前领导者
// type ShardLeaderResolver interface {
// 	// GetLeader 返回目标 Shard 的当前领导者 ReplicaID，NoLeader 表示无领导者
// 	GetLeader(shardID uint64) (leaderID uint64, err error)
// }
...
// ITransport 定义节点间通信接口，新增跨Shard领导者通信方法
type ITransport interface {
	...
	// SendMessageToShardLeader 向目标Shard的当前领导者发送消息
	// 参数：
	//   targetShardID: 目标Shard的ID
	//   msg: 待发送的Raft消息（Type/From/ShardID等字段需预先设置）
	// 返回：发送结果错误
	SendMessageToShardLeader(targetShardID uint64, msg pb.Message) error
}
