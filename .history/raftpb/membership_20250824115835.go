...
// 作用：这是 Raft 协议层面的成员信息结构体，存储分片的完整成员配置（包括投票节点、非投票节点、见证者等），由协议内部使用（如序列化/反序列化、成员变更）。
type Membership struct {
	ConfigChangeId uint64            // 最后一次成员变更的 Raft 日志条目索引
	Addresses      map[uint64]string // 常规 Raft 节点（投票成员）的 ID 到地址的映射
	Removed        map[uint64]bool   // 已从分片移除的节点 ID 集合
	NonVotings     map[uint64]string // 非投票成员的 ID 到地址的映射
	Witnesses      map[uint64]string // 见证者节点的 ID 到地址的映射
}
