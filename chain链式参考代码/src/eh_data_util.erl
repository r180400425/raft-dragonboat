...
queue_fun(Q0, _Acc0) ->
  queue:out(Q0).%取出队首元素，并返回剩下的队列

queue_r_fun(Q0, _Acc0) ->
  queue:out_r(Q0).%取出队尾元素，并返回剩下的队列

% 根据标志决定是否取出队列元素，用于数据校验
check_data_queue_fun(Q0, true) ->
  {empty, Q0};
check_data_queue_fun(Q0, false) ->
  queue:out_r(Q0). % 为什么是取队尾？

% 通用数据处理模板函数，递归处理队列中的数据（处理单个元素的函数，从队列中取出元素的函数，处理条件，输入队列，累加器）
...
% 根据时间戳和索引列表生成数据快照。返回一个按时间戳和索引排序的数据队列。
...
% 使用 maps:fold/3 遍历数据。
% 调用 snapshot_fun/3 处理每个条目。
% 使用 lists:sort/2 按时间戳和索引排序。
...
get_data_fun({ObjectType, ObjectId}, 
 ...
 % 获取指定对象的完整数据。返回包含时间戳、状态、列、值等信息的列表。
...
% 使用 queue_r_fun/2 从队列尾部开始处理。
% 调用 get_data_fun/3 提取数据。
